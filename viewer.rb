...
module Aims
  module Visualizer
    ...
    class Viewer
      ...
      # Set to true if invoked through glut
      # then glut refresh events are called
      attr_accessor :using_glut
...
=end
      ...
      def Viewer.visualize(unit_cell)

        v = Viewer.instance

        v.unit_cell = unit_cell
        ...
        glutCreateWindow("Aims Geometry Viewer")

        v.on_resize.call(v.width, v.height)
        glutDisplayFunc(v.on_draw)
        glutReshapeFunc(v.on_resize)
        glutMouseFunc(v.on_click)
        glutKeyboardFunc(v.on_key)
        #		glutTimerFunc(1000, v.timer_func, nil)
        glutMotionFunc(v.method(:mouse_dragged).to_proc)

        v.set_mouse_motion_function(:rotate)
      ...
      def on_resize
        Proc.new do |width, height|
          v = Viewer.instance
          v.viewport_setup(width, height)
        end
      end

      def on_click
        Proc.new do |button, state, x, y|
          v = Viewer.instance
          case state
          when GLUT_DOWN
            v.mouse_down(x,y)
      ...
      #          v = Viewer.instance
      #          v.current_cell += 1
      #          unless v.current_cell < v.unit_cell.size
      #            v.current_cell = 0
      #          end
      #          glutPostRedisplay
      #          glutTimerFunc(1000, v.timer_func, nil)        
